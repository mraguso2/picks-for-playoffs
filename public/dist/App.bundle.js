...
var showSubmit = exports.showSubmit = function submitButt() {
  var eid = this.name.split('_')[1];
  var submit = (0, _bling.$)('[data-score="' + eid + '"]');
...
var pickScore = exports.pickScore = function totalScore(e) {
  var year = window.location.href.slice(-4);
  if (e.keyCode === 8 || e.keyCode === 46) return;
  var eid = void 0;
  var scoreInput = void 0;
  ...
  if (e.keyCode === 13) {
    ...
    scoreInput = (0, _bling.$)('input[name="score_' + eid + '"]');
    if (!eid) return;
  }
  if (!eid) return;
  ...
  _axios2.default.post('/api/v1/pickScore/' + year + '/' + eid + '/' + score).then(function (res) {
    if (res.data.action === 'invalid') {
      window.scrollTo(0, 0);
      return location.reload();
    }
...
var preventPick = exports.preventPick = function preventPick() {
  var lock = (0, _bling.$)('[data-lock="' + (this.dataset.game || this.dataset.total) + '"] > .gameStatus');
  lock.classList.add('gameStatus__shake');
  setTimeout(function () {
    return lock.classList.remove('gameStatus__shake');
  }, 800);
};
...
// toggle menu
var menu = (0, _bling.$)('input[name="menu"]');
if (menu) {
  menu.on('click', _menu2.default);

  // toggle menu
  var content = (0, _bling.$)('.content');
  content.on('click', _menu2.default);
}

// find all the teams
var teams = (0, _bling.$$)('.team > input[type=radio]');
teams.on('click', _pick.pickTeam);

// find all the disabled radio teams
var disabledTeams = (0, _bling.$$)('.team.disabled');
disabledTeams.on('click', _pick.preventPick);

// find all the disabled number inputs
var disabledTotalScore = (0, _bling.$$)('.totalScore.disabled');
disabledTotalScore.on('click', _pick.preventPick);

// find all the total game Score inputs
var inputGameScores = (0, _bling.$$)('input[type=number]');
inputGameScores.on('input', _pick.showSubmit);
inputGameScores.on('keyup', _pick.pickScore);
...
function toggleMenu() {
  var navbar = (0, _bling.$)('nav.nav');
  var header = (0, _bling.$)('.top');

  if (this.classList.contains('content')) {
    var menu = (0, _bling.$)('#menu');
    if (menu) {
      menu.checked = false;
    }
